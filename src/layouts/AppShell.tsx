          ...
          <header className="space-y-3 border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-900">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                Queue: {syncStatus.pendingMutations}
              </span>
              <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                {syncStatus.isSyncing ? 'Syncing...' : syncStatus.lastSyncedAt ? `Last sync ${new Date(syncStatus.lastSyncedAt).toLocaleTimeString('id-ID')}` : 'Belum sync'}
              </span>
            </div>
            {syncStatus.conflictNotes.length > 0 ? (
              <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-200">
                <div className="mb-1 flex items-center justify-between gap-2">
                  <p className="font-semibold">Conflict notes ({syncStatus.conflictNotes.length})</p>
                  <button type="button" onClick={clearConflictNotes} className="rounded border border-amber-400 px-2 py-0.5 text-[11px]">Clear</button>
                </div>
                <ul className="list-disc space-y-0.5 pl-4">
                  {syncStatus.conflictNotes.slice(0, 2).map((note) => (
                    <li key={note}>{note}</li>
                  ))}
                </ul>
              </div>
