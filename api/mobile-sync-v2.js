...
async function upsertDataFile(entry) {
  ...
  try {
    const current = await githubRequest(`/repos/${owner}/${repo}/contents/${DEFAULT_DATA_PATH}?ref=${DEFAULT_BRANCH}`);
    sha = current.sha;
  ...
  await githubRequest(`/repos/${owner}/${repo}/contents/${DEFAULT_DATA_PATH}`, {
    method: 'PUT',
    body: JSON.stringify({
